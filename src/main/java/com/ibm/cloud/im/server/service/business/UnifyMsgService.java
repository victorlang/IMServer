...
public class UnifyMsgService {
	...
	private List <DiscussGroupInfo>  sortTopic(List <String> list, String userId)
	{
		...
		while(true)
		{
			if (list!=null && list.size()>0)
			{
				...
				for (TopicGroup topicObj: topicGroup)
				{
					...
					boolean bAccessible = topicGroupService.checkTopicUser(topicObj, userId);
					if(!bAccessible)
					{
						//无权限订阅的主题
						continue;
	...
	public void addGroup(String userId, String topicId, long type, boolean updateListSeq)
	{
		//updateListSeq=false， 
		//在订阅一个主题的时候， 如果该主题已经存在于用户数据的订阅列表中， 不会更新订阅列表顺序。
		//
		userTopicsDao.addGroup(userId, topicId, type,updateListSeq);
	}
	public void addGroup(List<String> userIds, String topicId, long type, boolean updateListSeq)
	{
	...
	public void addUserDiscussGroupId(List<String> userIds, String topicId, boolean updateListSeq)
	{
		for (String userId:userIds)
			userTopicsDao.addDiscussGroup(userId, topicId,updateListSeq);
	}
	public void addUserCaseGroupId(List<String> userIds, String topicId, boolean updateListSeq)
	{
		for (String userId:userIds)
			userTopicsDao.addCaseGroup(userId, topicId,updateListSeq);
	}
	public void addUserFileGroupId(List<String> userIds, String topicId,boolean updateListSeq)
	{
		for (String userId:userIds)
			userTopicsDao.addDiscussGroup(userId, topicId,updateListSeq);
	}
